...
def validate(_code):
    (name, number, checksum) = re.split("(\d+)", _code)
    name = name.replace("-", "")
    checksum = checksum.replace("[", "").replace("]", "")

    # all letters with count
    letter_count = []
    for letter in name:
        letter_count.append((letter, name.count(letter)))

    # sorted by count, and then alphabetically
    sorted_letter_count = sorted(letter_count, key=lambda l: (-l[1], l[0]))

    # get unique letters in order
    ranking = ''
    for letter in sorted_letter_count:
        if letter[0] not in ranking:
            ranking += letter[0]

    if ranking[:5] == checksum[:5]:
        return True


def validate_names(_codes):
    # find the valid names
    _codes = list(filter(lambda x: validate(x), _codes))
    return _codes


def decipher_names(_codes):
    deciphered = []
    for code in _codes:
...
def find_north_pole_objects(_rooms):
    for room in _rooms:
        if "north" in room[0]:
            print(room[0], room[1])


# test_1 = find_sector_sum(test_input_1)
# answer = find_sector_sum(codes)
#
# print("test_1:", test_1)
# print("answer:", answer)

find_north_pole_objects(decipher_names(validate_names(codes)))
