...
def distance(x1, y1, z1, x2, y2, z2):
    ...
    # return abs(x1 - x2) + abs(y1 - y2) + abs(z1 - z2)  # not this time, manhattan ;)


def solve(box_list, connection_limit):
    distances = {}
    ...
    for k, v in sorted(distances.items(), key=lambda x: x[1])[:connection_limit]:
        # pprint(circuits)
        # print(k, v)
...
def main():
    test_1 = solve(test_boxes, 10)
    print('test_1:', test_1)

    start = time()
    answer = solve(boxes, 1000)
